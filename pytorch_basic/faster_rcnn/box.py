...
def batched_nms(boxes: Tensor, scores: Tensor, idxs: Tensor,
    ...
    boxes: (N, 4) - proposals
    scores: (N,) - 预测的概率(用来排序的)
    idxs: (N, ) - 对应的feature map索引/类别, 不同层feature map/类别的boxes不会相互影响
    iou_threshold: float, IoU>iou_threshold的boxes会被删除
...
def clip_boxes_to_image(boxes: Tensor, size: Tuple[int, int]) -> Tensor:
    ...
    参数:
    boxes: (N, 4)或者(N, num_classes, 4)
    size: (H, W) padding前的图像尺寸

    返回:
    clipped_boxes: (N, 4)或者(N, num_classes, 4)
    """
    dim = boxes.dim()
    # boxes_x的形状: (N, 2)或者(N, num_classes, 2)
    boxes_x = boxes[..., 0::2]  # x1, x2
    # boxes_y的形状: (N, 2)或者(N, num_classes, 2)
    ...
    # clipped_boxes的形状: (N, 2, 2)或者(N, num_classes, 2, 2)
    clipped_boxes = torch.stack((boxes_x, boxes_y), dim=dim)
    # clipped_boxes的形状: (N, 4)或者(N, num_classes, 4)
